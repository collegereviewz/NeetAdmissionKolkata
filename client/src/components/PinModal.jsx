...
const centralKeywords = ['All India', 'Armed Forces', 'DNB', 'Open States', 'NEIGRIHMS'];

const bouncy = { type: 'spring', stiffness: 500, damping: 15, mass: 0.8 };

const PinModal = ({ isOpen, onClose, pinnedItems, togglePin }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [activeCategory, setActiveCategory] = useState('Central');

  if (!isOpen) return null;

  const filteredItems = counselingOptions.filter(option => 
    option.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const centralOptions = filteredItems.filter(opt => 
    centralKeywords.some(key => opt.includes(key))
  );
  const stateOptions = filteredItems.filter(opt => 
    !centralKeywords.some(key => opt.includes(key))
  );
              ...
              >
                <div className="flex-1 truncate mr-4">
                  <h4 className={`text-sm font-bold truncate transition-colors ${
                    pinnedItems.includes(option) ? 'text-college-primary' : 'text-gray-700'
                  }`}>
                    {option}
                  </h4>
                  <p className="text-[10px] text-gray-400 font-medium">NEET PG 2025-26</p>
                </div>
