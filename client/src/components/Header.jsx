...
const Header = ({ onOpenChoiceList, onLogout, onOpenMobileMenu, user }) => {
  ...
  return (
    ...
    <header className="header-gradient mx-4 md:mx-6 mb-4 md:mb-4 mt-4 md:mt-3 rounded-3xl px-4 md:px-8 py-3 md:py-2 flex items-center sticky top-4 md:top-3 z-40 gap-2 md:gap-4 lg:gap-6">
      ...
      <div className="relative" ref={dropdownRef}>
          ...
          >
              <span className="font-black text-[10px] md:text-xs tracking-wider uppercase">{selectedCourse}</span>
              <ChevronDown size={14} className={`ml-1 md:ml-2 transition-transform ${isCourseDropdownOpen ? 'rotate-180' : ''}`} strokeWidth={3} />
          </div>
                      ...
                      <div key={idx} className="mb-2 bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
                           ...
                           <div className="p-2 space-y-1 bg-white">
                              {category.courses.map((course) => {
                                  const isAvailable = course === 'NEET PG';
                                  return (
                                      <div 
                                          key={course}
                                          className={`flex items-center justify-between px-3 py-2 rounded-md transition-colors ${
                                              ...
                                              ? 'bg-blue-50' 
                                              : isAvailable ? 'hover:bg-gray-50 cursor-pointer' : 'opacity-60 cursor-not-allowed'
                                          }`}
                                          onClick={() => {
                                              if (isAvailable) {
                                      ...
                                      >
                                          <div className="flex flex-col">
                                              ...
                                              {!isAvailable && (
                                                  <span className="text-[9px] font-black text-blue-500 uppercase tracking-tight mt-0.5">Coming Soon</span>
                                              )}
                                          </div>
                                          {selectedCourse === course ? (
