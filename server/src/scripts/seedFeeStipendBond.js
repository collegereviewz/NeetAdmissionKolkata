...
const seedData = async () => {
    ...
    try {
        // Clear existing data
        await FeeStipendBond.deleteMany({});
        console.log("Cleared existing FeeStipendBond data");

        // Path to the fees data
        const dataDir = path.join(process.cwd(), 'src/data/fees/All India Counseling PG Medical');
        ...
        for (const file of files) {
            ...
            if (docs.length > 0) {
                await FeeStipendBond.insertMany(docs);
                totalDocs += docs.length;
                console.log(`Inserted ${docs.length} records from ${file}`);
            }
        }

        console.log(`SUCCESS: Seeded total ${totalDocs} FeeStipendBond records.`);
        process.exit();
    } catch (error) {
